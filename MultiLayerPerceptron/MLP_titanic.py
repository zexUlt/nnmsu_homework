...
#    эта матрица имеет кол-во строк на единицу больше т.к. нужно учитывать мнимую единицу
    ...
    # w3 = np.ones((hiddens2 + 1, hiddens3))
...
#            добавление в начало вектора мнимой единицы
            ...
            # hidden_in3 = np.dot(hidden_out2, w3)
            # hidden_out3 = f(hidden_in3)
            # hidden_out3 = np.array(np.insert(hidden_out3, 0, [1]), ndmin=2)
...
#             hidden_error3 = np.dot(output_error, w3.T)
...
#            y` = 1 в dW = lr*output_error*hidden_out2.T не умножается на эту производную
            ...
            # w3 += lr * hidden_error3[:, 1:] * f1(hidden_out3[:, 1:]) * hidden_out2.T
...
max_iter = 14*14
current_iter = 1
search_dict = dict({"hidden layer 1": list(), "hidden layer 2": list(), "score": list(), "era": list(), "eps": list()})
# for l in np.arange(.6, 2, .1):
# for hl in range(1, 15):
#      for hl2 in range(1, 15):
print("Iteration %d/%d" % (current_iter, max_iter))
current_iter += 1
# инициализация весов в зависимости от количества узлов в слоях сети
w1, w2, w3 = init_weight(input_layer, 10, 2, output_layer)

# search_dict["hidden layer 1"].append(hl)
# search_dict["hidden layer 2"].append(hl2)
# тренировка сети
# train network
w1, w2, w3, era, lst = train(inputs, w1, w2, w3, targets, lr, eps)

search_dict["era"].append(era)
search_dict["eps"].append(lst[-1])
...
search_dict["score"].append(str(eq*100))
# вывод точности
print("Результат тестирования (в %) = " + str(eq*100))
# pd.DataFrame.from_dict(search_dict).to_csv("results_0695.csv")
